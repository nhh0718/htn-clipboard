...
function createActionBtn(iconHtml, title, onClick) {
...
async function handlePin(id) {
    const res = (await sendMsg({ action: 'pin', id }));
    if (res?.error) {
        showToast('Error: ' + res.error);
    }
    else {
...
async function handleDelete(id) {
    const res = (await sendMsg({ action: 'delete', id }));
    if (res?.error) {
        showToast('Error: ' + res.error);
    }
    else {
...
async function fetchAndRender() {
    ...
    try {
        ...
        if (hasFilter) {
            ...
            res = (await sendMsg({ action: 'getHistory', limit: 30, offset: 0 }));
        }
        if (res?.error) {
