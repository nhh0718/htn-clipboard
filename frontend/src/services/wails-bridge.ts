...
export function GetVersion(): Promise<string> {
  return inWails() ? goApp().GetVersion() : Promise.resolve('dev')
}
...
export function OpenUpdatePage(url: string): Promise<void> {
  return inWails() ? goApp().OpenUpdatePage(url) : Promise.resolve()
}
